...
import { Box, Card, CardActionArea, CardContent, Grid2, Stack, Typography, useMediaQuery, useTheme, MenuItem, Autocomplete } from '@mui/material'
...
const RegistrationPage = () => {
  ...
  const options = dataProvince.flatMap(provinsi => provinsi.kota.map(kota => ({
    label: kota,
              ...
              <form>
                <Stack spacing={3}>
                      ...
                      activeClass.Classes.filter((c) => c.program_id === selectedProgram).length > 0 ? (
                        activeClass.Classes.filter((c) => c.program_id === selectedProgram).map((kelas) => (
                          <MenuItem key={kelas.class_id} value={kelas.class_id} sx={{ whiteSpace: "normal" }}>
                            {kelas.class_name} - Batch {activeClass.Batches.find(b => b.batch_id === kelas.batch_id)?.batch_number} ({format(activeClass.Batches.find(b => b.batch_id === kelas.batch_id)!.batch_start, "dd MMMM yyyy", { locale: id })})
                          </MenuItem>
                        ))
                      ) : (
                  ...
                  <Grid2 container spacing={2}>
                    ...
                    <CustomTextField label="Nama Lengkap Sesuai KTP" variant="outlined" fullWidth name="name" id="name" required />
                    <CustomTextField label="Email" variant="outlined" fullWidth name="email" id="email" type="email" required />
                    <CustomTextField label="Jenis Kelamin" variant="outlined" fullWidth name="gender" id="gender" select required >
                      <MenuItem value="" disabled>Pilih Jenis Kelamin</MenuItem>
                      <MenuItem value="Laki-laki">Laki-laki</MenuItem>
                      <MenuItem value="Perempuan">Perempuan</MenuItem>
                    </CustomTextField>
                    <Grid2 size={{ xs: 5, sm: 6 }}>
                      <CustomTextField label="Tempat Lahir" variant="outlined" fullWidth name="place_of_birth" id="place_of_birth" required />
                    </Grid2>
                    ...
                    <CustomTextField label="Pendidikan Terakhir" variant="outlined" fullWidth name="pendidikan" id="pendidikan" select required >
                      <MenuItem value="" disabled>Pendidkan Terakhir</MenuItem>
                    ...
                    <Grid2 size={{ xs: 6, sm: 6 }}>
                      <CustomTextField label="Nomor HP" variant="outlined" fullWidth name="phone" id="phone" type="tel" required placeholder="Contoh: 081234567890" inputProps={{ maxLength: 15 }} />
                    </Grid2>
                    <Grid2 size={{ xs: 6, sm: 6 }}>
                      <CustomTextField label="Nomor HP Kerabat (Tidak Serumah)" variant="outlined" fullWidth name="phone" id="phone" type="tel" required placeholder="Contoh: 081234567890" inputProps={{ maxLength: 15 }} />
                    </Grid2>
                    <Autocomplete
                      options={options}
                      ...
                      renderInput={(params) => (
                        <CustomTextField {...params} label="Kota Domisili" variant="outlined" />
                      )}
                      fullWidth

                    />
                    <CustomTextField label="Alamat Lengkap" variant="outlined" fullWidth name="address" id="address" required multiline rows={4} placeholder="Masukkan alamat lengkap" />
                    <CustomTextField
